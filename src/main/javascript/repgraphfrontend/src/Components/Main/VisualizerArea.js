...
const testGraphNodeSpanLengths = testGraph.nodes
  .map((node) => node.anchors[0])
  .map((span) => span.end - span.from);

let uniqueSpans = [];
const map = new Map();
for (const item of testGraphNodeSpanLengths) {
  if (!map.has(item)) {
    map.set(item, true); // set any value to Map
    uniqueSpans.push(item);
  }
}
uniqueSpans.sort((a, b) => a - b); //sort unique spans ascending

function getNodeLevel(id) {
  const nodeSpan = testGraphNodeSpanLengths[id];
  return uniqueSpans.findIndex((element) => element === nodeSpan);
}

const testGraphNodes = testGraph.nodes.map((node) => ({
  id: node.id,
  x: node.id * 90,
  y: 400 - getNodeLevel(node.id) * (400 / (uniqueSpans.length - 1)),
  label: node.label,
  type: "node",
}));
...
const testGraphData = {
  nodes: testGraphNodes.concat(testGraphTokens),
      ...
      <Paper style={styles.Paper} variant="elevation" elevation={5}>
        ...
        <TextareaAutosize
          ...
          value={
            ...
            "\n" +
            JSON.stringify(testGraphNodeSpanLengths)
          }
        />
